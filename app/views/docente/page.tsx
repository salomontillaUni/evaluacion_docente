...
export default function TeacherView() {
  const [selectedPeriodo, setSelectedPeriodo] = useState(
    periodosAcademicos[0].id,
  );
  const [isPeriodoOpen, setIsPeriodoOpen] = useState(false);

  const currentPeriodo = periodosAcademicos.find(
    (p) => p.id === selectedPeriodo,
  );
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Consulta tus evaluaciones por período
            </p>
          </div>
        </div>
        ...
        <div className="relative w-full md:w-72">
          ...
          >
            <span className="text-sm text-foreground">
              {currentPeriodo?.label}
            </span>
            <ChevronDown
              className={`w-4 h-4 text-muted-foreground transition-transform ${isPeriodoOpen ? "rotate-180" : ""}`}
            />
          </button>
          {isPeriodoOpen && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          <p className="text-xs text-muted-foreground mb-6">
            Análisis porcentual de opiniones recibidas
          </p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart
              data={sentimientosDistribucion}
              layout="vertical"
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              ...
              <ul className="text-sm text-emerald-700 space-y-1.5">
                <li className="flex items-start gap-2">
                  ...
                  Excelente capacidad de comunicación y claridad en las
                  explicaciones
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
                  Alta disponibilidad para resolver dudas fuera de horario de
                  clase
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
                  Uso de ejemplos prácticos y laboratorios bien estructurados
                </li>
              </ul>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <ul className="text-sm text-amber-700 space-y-1.5">
                <li className="flex items-start gap-2">
                  ...
                  Mayor claridad en los criterios de evaluación y rúbricas
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />
                  Incrementar retroalimentación oportuna en proyectos y tareas
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />
                  Incorporar más actividades de trabajo colaborativo
                </li>
              </ul>
            </div>
          </div>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Todas las evaluaciones son anónimas
        </p>
          ...
          {comentariosDocente.map((c, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border ${getSentimentBadge(c.sentimiento)}`}
                >
